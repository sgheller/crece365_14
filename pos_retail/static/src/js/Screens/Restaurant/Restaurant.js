odoo.define('pos_retail.restaurant', function (require) {
    ...
    models.PosModel = models.PosModel.extend({
        ...
        async set_table(table, order) {
            ...
            if (table && table.locked) {
                ...
                let validate = await this._validate_by_manager(this.env._t('This Table is Locked, Need Manager unlock it'));
                if (validate) {
    ...
    models.Order = models.Order.extend({
        ...
        saveChanges: function (locked_sync) { // TODO: set all line to Waiting and push event to pos kitchen screen
            ...
            let orderReceipt = this.buildReceiptKitchen(this.computeChanges());
            const res = _super_order.saveChanges.apply(this, arguments);
            if ((this.syncing == false || !this.syncing) && this.pos.pos_bus && !this.pos.splitbill && (orderReceipt['new'].length > 0 || orderReceipt['cancelled'].length > 0)) {
                this.pos.pos_bus.requests_printers.push({
    ...
    _super_order.computeChanges = function (categories) {
        ...
        this.last_sync = {
            'request_number': this.pos.db.getRequestKitchenNumber(),
