        ...
        class extends TicketScreen {
            ...
            async saveToPartialOrder(selectedOrder) {
                ...
                let {confirmed, payload: paymentMethod} = await this.showPopup('SelectionPopup', {
                    title: this.env._t('Are you want save Order with Partial Mode ?'),
                    body: this.env._t('This Feature allow save Order to draft and allow register one part of Total Order Amount'),
                    list: lists
                })
                if (confirmed) {
                    ...
                    if (confirmed) {
                        ...
                        if (number < 0 || number > selectedOrder.get_total_with_tax()) {
                            return this.showPopup('ErrorPopup', {
                                title: this.env._t('Error'),
                                body: this.env._t('Register payment amount required bigger than 0 and smaller than Order Total Amount')
                            })
                        }
                        if (number > 0) {
                        ...
                        this.showPopup('ConfirmPopup', {
                            title: this.env._t('Successfully Save Order !!!'),
