            ...
            async apply_discount(pc) {
                ...
                if (amountTotalWithTaxes != amountTotalWithOutTaxes) {
                    let {confirmed} = await this.showPopup('ConfirmPopup', {
                        title: this.env._t('Alert, Please choose 1 option'),
                        body: this.env._t('If you need Discount compute base on Amount total included taxes, choice WithIn Taxes, else choose WithOut Taxes'),
                        confirmTextButton: this.env._t('WithIn Taxes'),
                        cancelTextButton: this.env._t('WithOut Taxes'),
                    })
                    if (confirmed) {
