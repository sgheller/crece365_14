odoo.define('pos_retail.GiftCardScreen', function (require) {
    ...
    class GiftCardScreen extends PosComponent {
        ...
        async askCustomer() {
            this.env.pos.db.notification('danger', this.env._t('Create will create will apply to customer ? Please select one'))
            ...
            if (confirmed) {
                if (this.env.pos.config.msg_before_card_pay) {
                    ...
                    if (confirmed) {
                        ...
                        this.showScreen('PaymentScreen', {'showGiftButton': false});
                        this.env.pos.db.notification('success', this.env._t('Please do payment, Card will active after Payment successfully !!!'))
                    }
                } else {
                    this.env.pos.get_order().set_is_rounding(false);
