    ...
    models.Order = models.Order.extend({
            ...
            if (confirmed) {
                ...
                for (let index in selectedCrossItems) {
                    ...
                    if (product) {
                        ...
                        this.pos.db.notification('danger', item['product_id'][1] + _t(' not available in POS'))
                    }
                }
            }
        },
        ...
        set_client: async function (client) {
            ...
            if (client && !this.pos.the_first_load) {
                if (client.balance < 0) {
                    this.pos.chrome.showPopup('ErrorPopup', {
                        title: _t('Warning'),
                        body: client.name + _t('has Credit Balance : ' + this.pos.format_currency(client.balance) + '. It mean Credit Amount - Debit Amount of this customer smaller than 0.')
                    })
                }
                ...
                if (partial_payment_orders.length != 0) {
                    ...
                    if (this.pos.chrome) {
                        this.pos.chrome.showPopup('ConfirmPopup', {
                            ...
                            body: this.pos.env._t('Have total ') + partial_payment_orders.length + this.pos.env._t(' Orders not paid full. Please processing full fill payment Amount: ') + this.pos.format_currency(due_amount),
                            disableCancelButton: true,
                        })
                        const link = window.location.origin + "/web#id=" + partial_payment_orders[0]['id'] + "&view_type=form&model=pos.order";
        ...
        async set_discount_value(discount) {
            ...
            if (discount <= 0) {
                return this.pos.chrome.showPopup('ErrorPopup', {
                    title: _t('Error'),
                    body: _t('It not possible set Discount is 0')
                })
            }
            ...
            const amount_withtax = this.get_total_with_tax();
            lines = lines.filter(l => l.get_price_with_tax() > 0)
            if (this.pos.config.discount_limit && this.pos.config.discount_value_limit < discount) {
                ...
                if (!confirm) {
                    ...
                    for (let i = 0; i < lines.length; i++) {
                        let line = lines[i];
                        let percent_disc = line.get_price_with_tax() / amount_withtax
                        line.price_extra = -percent_disc * discount;
                        line.trigger('change', line)
            ...
            if (promotions.length) {
                ...
                for (let i = 0; i < promotions.length; i++) {
                    ...
                    if (order.orderlines.length) {
                        ...
                        this.pos.db.notification('success', _t('Promotion Program: ') + promotion['name'] + _t(' Applied to Order!!!'));
                    }
                }
    ...
    models.Orderline = models.Orderline.extend({
        ...
        hasPriceOfLineIsValid() {
            ...
            var pricelist_items = _.filter(pricelist.items, function (item) {
                return (!item.product_tmpl_id || item.product_tmpl_id[0] === product.product_tmpl_id) &&
                    ...
                    (item['min_price'] && item['min_price'] >= 0 && item['max_price'] && item['max_price'] >= 0 && item['min_price'] < item['max_price']) &&
                    (self.price < item['min_price'] || item['max_price'] < self.price);
            });
            if (!pricelist_items || pricelist_items.length == 0) {
                return []
            } else {
        ...
        set_unit: function (uom_id, price) {
            ...
            if (price != 0) {
                ...
                let newPrice = this.product.get_price(this.pos._get_active_pricelist(), this.quantity, this.uom_id);
